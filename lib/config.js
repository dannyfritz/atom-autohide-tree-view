...
import {updateTreeView, updateTriggerArea, showTreeView, disableAutohide} from './autohide-tree-view.js';
...
export const schema = {
  ...
  pinnedByDefault: {
    description: 'Tree is pinned by default.',
    type: 'boolean',
    default: false,
    order: 11
  }
...
export function observeConfig() {
  return new CompositeDisposable(
    ...
    atom.config.observe('autohide-tree-view.pinnedByDefault', pinnedByDefault => {
      if (pinnedByDefault) {
        showTreeView()
        disableAutohide()
        pinView.activate()
